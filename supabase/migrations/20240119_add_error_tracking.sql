-- Migration: Add error tracking to notes table and create processing_errors table
-- Date: 2024-01-19

-- Add error tracking columns to notes table
ALTER TABLE notes 
...
CREATE INDEX idx_notes_error_status ON notes(error_message) WHERE error_message IS NOT NULL;
CREATE INDEX idx_notes_processing_attempts ON notes(processing_attempts);
CREATE INDEX idx_notes_last_error_at ON notes(last_error_at);
...
-- RLS policy for notes (existing users can only see their own notes)
CREATE POLICY "Users can view their own notes with errors" ON notes
    FOR SELECT USING (auth.uid() = user_id);

CREATE POLICY "Users can update their own notes with errors" ON notes
    FOR UPDATE USING (auth.uid() = user_id);

-- RLS policy for processing_errors (users can only see errors for their own notes)
CREATE POLICY "Users can view processing errors for their notes" ON processing_errors
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM notes 
