...
export default function NoteCard({ note, onDelete, onRefresh, highlightFilter }: NoteCardProps) {
  ...
  const getStatusIcon = () => {
    if (note.error_message) {
      return (
        <svg className="h-4 w-4 text-red-500" fill="currentColor" viewBox="0 0 20 20" title="Processing failed">
    ...
    if (note.transcription && note.analysis) {
      return (
        <svg className="h-4 w-4 text-green-500" fill="currentColor" viewBox="0 0 20 20" title="Processing completed">
    ...
    if (note.processing_started_at && !note.processed_at) {
      return (
        <svg className="h-4 w-4 text-yellow-500 animate-spin" fill="none" viewBox="0 0 24 24" title="Currently processing">
    ...
    if (note.transcription && !note.analysis) {
      return (
        <svg className="h-4 w-4 text-orange-500 animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor" title="Analyzing transcription">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
        </svg>
      )
    }
    return (
      <svg className="h-4 w-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" title="Queued for processing">
